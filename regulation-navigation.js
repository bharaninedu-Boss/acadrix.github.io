...
handleHashRoute=function(){
    ...
    if(isMechanical(dept)&&/^r202[15]$/i.test(p[2]||'')){
        ...
        return navigateTo('details',{dept,sem,regulation,subjectCode:decodeURIComponent(p[4])},true);
    }
    const sm=(p[2]||'').match(/^sem(\d+)$/i);
    if(sm){const sem=parseInt(sm[1],10);if(!p[3])return navigateTo('subjects',{dept,sem,regulation:'r2021'},true);return navigateTo('details',{dept,sem,regulation:'r2021',subjectCode:decodeURIComponent(p[3])},true);}
...
renderSemesters=function(container,deptId,regulation=null){
    ...
    const label=regulationLabel(chosen);container.innerHTML=`<div class="breadcrumb"><span onclick="navigateTo('home')">Home</span> › <span onclick="navigateTo('semesters',{dept:'mech'})">Mechanical Engineering</span> › <span>${label}</span></div><section class="hero"><h1>Mechanical Engineering — ${label}</h1><p>${chosen==='r2025'?'R-2025 resources are stored separately from R-2021 and will be expanded semester-by-semester.':'R-2021 resources and existing study material.'}</p></section><section><h2>Select Semester</h2><div class="grid">${[1,2,3,4,5,6,7,8].map(n=>`<div class="card" role="button" tabindex="0" onclick="navigateTo('subjects',{dept:'mech',sem:${n},regulation:'${chosen}'})"><div><h3>Semester ${n}</h3><p style="margin:0;color:var(--text-secondary)">Mechanical Engineering · ${label}</p></div><div class="arrow">→</div></div>`).join('')}</div></section>`;
};
